...
actor {
  ...
  // Inventory Management (as updateStock) - Admin Only
  public shared ({ caller }) func updateStock(productId : Text, stock : Nat) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
  ...
  public query func getAllProductsSortedByStock() : async [Product] {
    products.values().toArray().sort(
      func(p1, p2) {
        if (p1.stock < p2.stock) { #less } else if (p1.stock == p2.stock) {
          #equal;
        } else { #greater };
      }
    );
  ...
  public shared ({ caller }) func updateCartQuantity(productId : Text, quantity : Nat) : async () {
    ...
    if (product.stock < quantity) {
      Runtime.trap("Not enough stock available for requested quantity");
    };
